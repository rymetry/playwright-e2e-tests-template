...
- `PLAYWRIGHT`: DOM、ARIA、navigation、networkなどをPlaywright Testで自動実行する
- `API`: Playwright request等でAPI／サービス層を自動実行する（ブラウザ非依存）
- `COMPUTER_USE`: Canvas、OS UI、ブラウザ拡張機能など、Playwrightだけでは
  十分に操作・判定できない対象を画面操作で実行する
- `MANUAL`: 人による意味判断、感性的評価、物理操作などが必要な対象を手動実行する
